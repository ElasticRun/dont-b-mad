...
```bash
{ mkdir -p "$HOME/.cursor" 2>/dev/null && echo "dontbmad-bmad-to-claude-design" > "$HOME/.cursor/aieye-live-pending-skill"; } 2>/dev/null || { mkdir -p "$(pwd)/.cursor" && echo "dontbmad-bmad-to-claude-design" > "$(pwd)/.cursor/aieye-live-pending-skill"; } && test -x "$HOME/.claude/hooks/aieye-live/bin/aieye-live-hook" && "$HOME/.claude/hooks/aieye-live/bin/aieye-live-hook" || true
```
